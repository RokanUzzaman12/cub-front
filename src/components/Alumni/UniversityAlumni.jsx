...
import {
  fetchAlumniAndFillStore,
  fetchDepartmentWiseAlumniAndFillStore,
  fetchFacultyWiseAlumniAndFillStore,
} from "../../loaders/alumniLoader.js";
import {
  alumniList,
...
const UniversityAlumni = ({
  slug = null,
  departmentAlumni = false,
  facultyAlumni = false,
}) => {
  let allAlumni = [];
  const Alumni = useStore(alumniList);
  const deptAlumni = useStore(departmentWiseAlumni) || [];
  const facAlumni = useStore(facultyWiseAlumni) || [];
  if (Alumni.length > 0) {
    allAlumni = Alumni;
  }
  if (deptAlumni.length > 0) {
    allAlumni = deptAlumni;
  }
  if (facAlumni.length > 0) {
    allAlumni = facAlumni;
  }
  ...
  useEffect(() => {
    if (slug == null) {
      ...
      if (facultyAlumni) {
        fetchFacultyWiseAlumniAndFillStore(slug);
      }
    }
  }, []);
  return (
                  ...
                  <div className="blog single-blog__content">
                    ...
                    <div className="blog__meta">
                      ...
                      <h5 className="blog__title">
                        <a href={`/alumni/${item.id}`}>{item.title}</a>
                      </h5>
                      <a
                        href={`/alumni/${item.id}`}
...
export default UniversityAlumni;
