...
const FacultyEmployees = ({ category, faculty, search = "" }) => {
  const storeKey = `${faculty}:${category}`;
  const allStaffs = useStore(facultyCategoryWiseStaffs);
  const facultyStaffs = allStaffs[storeKey] || [];
  useEffect(() => {
    fetchFacultyCategoryWiseStaffsAndFillStore(faculty, category);
  }, [faculty, category]);
  // filter by search term (name or designation)
