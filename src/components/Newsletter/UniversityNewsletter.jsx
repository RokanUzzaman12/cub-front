...
import {
  fetchNewsletterAndFillStore,
  fetchDepartmentWiseNewsletterAndFillStore,
  fetchFacultyWiseNewsletterAndFillStore,
} from "../../loaders/newsletterLoader.js";
import {
  newsletterList,
...
const UniversityNewsletter = ({
  slug = null,
  departmentNewsletter = false,
  facultyNewsletter = false,
}) => {
  let allNewsletter = [];
  const Newsletter = useStore(newsletterList);
  const deptNewsletter = useStore(departmentWiseNewsletter) || [];
  const facNewsletter = useStore(facultyWiseNewsletter) || [];
  if (Newsletter.length > 0) {
    allNewsletter = Newsletter;
  }
  if (deptNewsletter.length > 0) {
    allNewsletter = deptNewsletter;
  }
  if (facNewsletter.length > 0) {
    allNewsletter = facNewsletter;
  }
  useEffect(() => {
    if (slug == null) {
      fetchNewsletterAndFillStore();
    } else {
      if (departmentNewsletter) {
        fetchDepartmentWiseNewsletterAndFillStore(slug);
      }
      if (facultyNewsletter) {
        fetchFacultyWiseNewsletterAndFillStore(slug);
      }
    }
  }, []);
  return (
                  ...
                  <div className="blog single-blog__content">
                    ...
                    <div className="blog__meta">
                      ...
                      <h5 className="blog__title">
                        <a href={`/newsletter/${item.id}`}>{item.title}</a>
                      </h5>
                      <a
