...
import { fetchActivitiesAndFillStore } from "../../loaders/activitesLoader";
import { activityList } from "../../stores/activitiesStore";
import {
  departmentWiseActivities,
  facultyWiseActivities,
} from "../../stores/activitiesStore.js";
import {
  fetchDepartmentWiseActivitiesAndFillStore,
  fetchFacultyWiseActivitiesAndFillStore,
} from "../../loaders/activitesLoader.js";

const UniversityActivities = ({
  slug = null,
  departmentActivity = false,
  facultyActivity = false,
}) => {
  let allActivities = [];
  const ACTIVITIES = useStore(activityList);
  ...
  if (departmentActivities.length > 0) {
    allActivities = departmentActivities;
  }
  if (facultyActivities.length > 0) {
    allActivities = facultyActivities;
  }
  useEffect(() => {
    if (slug == null) {
      fetchActivitiesAndFillStore();
    } else {
      if (departmentActivity) {
                  ...
                  <div className="blog single-blog__content">
                    ...
                    <div className="blog__meta">
                      ...
                      <h5 className="blog__title">
                        <a href={`/activities/${item.id}`}>{item.title}</a>
                      </h5>
                      <a
