...
const OfficeMenu = () => {
  let departmentData = useStore(departmentList);
  departmentData = departmentData.filter((dept) => dept.is_office === true);
  useEffect(() => {
        ...
        <ul className="submenu mm-collapse">
          {departmentData.map((department) => (
            <li key={department.id}>
              <a href={`/departments/${department.slug}`}>{department.name}</a>
            </li>
          ))}
        </ul>
      </li>
