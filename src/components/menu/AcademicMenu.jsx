...
export default function AcademicsMenu() {
  let facultyLists = useStore(facultyList);
  facultyLists = facultyLists.filter((dept) => dept.is_office === false);
  useEffect(() => {
      ...
      <li className="navigation__menu--item has-child has-arrow">
        ...
        <ul className="submenu sub__style">
          {facultyLists.map((faculty) => (
            <li
              key={faculty.id}
              className={faculty.departments.length ? "has-child" : ""}
            >
              <a href={`/faculties/${faculty.slug}`}>{faculty.name}</a>
