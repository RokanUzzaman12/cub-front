...
import { fetchResearchAndFillStore } from "../../loaders/researchLoader";
import { researchList } from "../../stores/researchStore";
import {
  departmentWiseResearch,
  facultyWiseResearch,
} from "../../stores/researchStore.js";
import {
  fetchDepartmentWiseResearchAndFillStore,
  fetchFacultyWiseResearchAndFillStore,
} from "../../loaders/researchLoader.js";

const UniversityResearch = ({
  slug = null,
  departmentResearch = false,
  facultyResearch = false,
}) => {
  let allResearch = [];
  const RESEARCHES = useStore(researchList);
  const departmentResearches = useStore(departmentWiseResearch) || [];
  const facultyResearches = useStore(facultyWiseResearch) || [];
  if (RESEARCHES.length > 0) {
    allResearch = RESEARCHES;
  }
  if (departmentResearches.length > 0) {
    allResearch = departmentResearches;
  }
  if (facultyResearches.length > 0) {
    allResearch = facultyResearches;
  }
  useEffect(() => {
    if (slug == null) {
      fetchResearchAndFillStore();
    } else {
